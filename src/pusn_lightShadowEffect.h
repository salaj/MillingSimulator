...
namespace pusn
{
	...
	{
	public:
		LightShadowEffect(pusn::DeviceHelper& device, std::shared_ptr<ID3D11InputLayout>& layout,
	...
	private:
		...
		std::shared_ptr<pusn::CBMatrix> m_mapMtxCB;
		...
		void InitializeTextures(pusn::DeviceHelper& device);
	};
}