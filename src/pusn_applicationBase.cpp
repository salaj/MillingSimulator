...
namespace pusn
{
	...
	void ApplicationBase::SetModelsManager(gk2::ModelsManager* modelsManager)
	{
	...
	bool ApplicationBase::Initialize()
	{
		...
		if (!TwInit(TW_DIRECT3D11, m_device.getDeviceObject().get()))
		{
			MessageBoxA(m_mainWindow->getHandle(), TwGetLastError(), "AntTweakBar initialization failed", MB_OK | MB_ICONERROR);
			//Cleanup();
			return false;
		}
