...
void Puma::InitializeHeightMap(int n, int m)
{
...
void Puma::fillIndices(int &index, int index1, int index2, int index3, int index4)
{
...
void Puma::InitializeDynamicBuffers()
{
	...
	InitializeHeightMap(materialWidth, materialHeight);
	...
	for (int j = 0; j < materialHeight; j++)
	{
		for (int i = 0; i < materialWidth; i++)
		{
			...
			cubeVertices[index].Normal = XMFLOAT3(0.0, 0.0, 1.0);
			index++;
		}
	}
	...
	m_dbVertexes.reset(new DBVertexes(m_device, verticesCount));
	indicesPointerMesh = m_device.CreateIndexBuffer(cubeIndicesMesh, indicesMeshCount);
	indicesPointerMaterial = m_device.CreateIndexBuffer(cubeIndicesMaterial, indicesMaterialCount);
...
void Puma::InitializePuma()
{
	for (int i = 0; i < 6; i++)
	{
		...
		m_vbPuma[i] = m_device.CreateVertexBuffer(&vertices[i][0], differentVertexCount, D3D11_USAGE_DYNAMIC);
...
bool Puma::LoadContent()
{
	...
	TwAddButton(bar, "Run", RunCB, getMainWindow(), " label='Otwórz plik' ");
...
void Puma::DrawCube()
{
	...
	m_dbVertexes->Update(m_context, cubeVertices);
	ID3D11Buffer* b = m_dbVertexes->getBufferObject().get();
	m_context->IASetVertexBuffers(0, 1, &b, &VB_STRIDE, &VB_OFFSET);
	m_context->IASetIndexBuffer(indicesPointerMesh.get(), DXGI_FORMAT_R16_UINT, 0);
	m_context->DrawIndexed(indicesMeshCount, 0, 0);

	//m_context->IASetPrimitiveTopology(D3D11_PRIMITIVE_TOPOLOGY_TRIANGLELIST);
	//m_context->IASetIndexBuffer(indicesPointerMaterial.get(), DXGI_FORMAT_R16_UINT, 0);
	//m_context->DrawIndexed(indicesMaterialCount, 0, 0);
}
